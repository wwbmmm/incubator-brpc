...
// Initialize read-write lock `rwlock'. `attr' is accepted for POSIX
// compatibility but currently ignored: no read-write lock attribute API
// is implemented, so passing nullptr is equivalent.
...
// NOTE: POSIX-style read-write lock attribute functions
// (bthread_rwlockattr_init/destroy/getkind_np/setkind_np) were once
// declared here but never implemented in libbrpc, causing undefined
// references at link time. They have been removed. `attr' of
// bthread_rwlock_init() is ignored (see above).
...
// NOTE: POSIX-style barrier functions (bthread_barrier_init/destroy/wait,
// a new feature in 1003.1j-2000) were once declared here but never
// implemented in libbrpc, causing undefined references at link time.
